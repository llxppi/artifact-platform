...
function SceneCard(artifact: Artifact) {
  return (
    <Link key={artifact.id} href={`/scene/${artifact.id}`}
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="ml-0 md:ml-64 pt-20 pb-28 md:pb-20 max-w-[1200px] mx-auto px-6">
        <div className="mb-8">
          <h2 className="text-4xl font-extrabold text-primary tracking-tighter mb-2">时光故事</h2>
          <p className="text-secondary text-sm">选择一件文物，生成专属情景故事</p>
        </div>
        <ArtifactBrowser renderCard={SceneCard} />
      </main>
    </div>
  );
}
